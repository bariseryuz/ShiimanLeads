...
async function buildAutoLeadQuickRead(input) {
  ...
  const ctx =
    ...
    `Tabular rows actually returned in this run: ${leads.length}\n` +
  ...
  const prompt =
    ...
    '- If sources were found, frame them as good places to dig; if zero rows came back, say that plainly and encourage refining the area or using saved sources for ongoing monitoring — without sounding like an error dump.\n' +
